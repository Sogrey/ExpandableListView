...
//		return getGenericView( this.list.get(groupPosition).getSection_name());
		...
		if (isExpanded) {
			...
			if (isExpanded||(!"00".equals(this.list.get(groupPosition).getParent_code())&&groupPosition!=this.list.size()-1)) {//末尾
...
//		intentChapter.setAction(Constants.RECEIVER_EXAM_PAPER_INFO);// action与接收器相同
